...
# Spotted_FI: Stock boundary for the 4-Islands (Maui Nui) population of spotted dolphins.
...
# Bottlenose_OUFI: Collective stock boundaries for the adjacent Oahu and 4-Islands (Maui Nui) populations of bottlenose dolphins.
...
- **`truncation_km:`**  Specifies the maximum perpendicular distance for groups that could potentially be included for abundance estimation. This is not the stage at which you set the truncation distance for detection function modeling; it is simply a preliminary cutoff for sightings made at unrealiable detection distances. The default is set at 20 km, which is farther than all sightings on record during Pacific NOAA Fisheries surveys. 
