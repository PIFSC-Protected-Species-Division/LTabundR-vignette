...
library(ggplot2)
library(LTabundR)
#devtools::document('../LTabundR')
reproc_cruz_from_scratch <- FALSE
rerun_eez_from_scratch <- FALSE
rerun_nwhi_from_scratch <- FALSE
rerun_covar_from_scratch <- FALSE
...
```{r, echo=TRUE, eval=reproc_cruz_from_scratch, collapse=TRUE, message=FALSE, warning=FALSE}
# Gather settings
...
```{r, echo=FALSE, eval=reproc_cruz_from_scratch, collapse=TRUE, message=FALSE, warning=FALSE}
...
The only column that is *required* here is `PerpDistKm`, but we are keeping date and coordinate columns as well. We are also including two additional columns, `Bft` and `EffType`, since we are going to use them in an example further down the page.  

Note that all column names within `df_sits` must be within `density_sightings`, another of the required inputs, and those names must match exactly. (That is, `density_sightings` can have many more columns than does `df_sit`, but all columns from the latter must be present in the former.) This is needed in order to predict the Effective Strip Width of `density_sightings` based on the detection function fitting procedure.  
...
This input is optional. It must be a named `list` containing parameters for fitting the detection function. See detailed documentation on the [Line Transect Analysis page](#dfsets). Default inputs shown below. Note that the default is that covariates are not used in detection function fitting. Briefly, here is an example of what this list should look like: 
...
If the slot `$covariates` is `NULL` (the default), then the detection function will be modeled without using any environmental covariates. To model with covariates, supply a character vector of column names that occur in **both** the inputs `df_sits` and `density_sightings`.  Explanations of the remaining slots are provided on [here](#dfsets).  
...
Note that all column names within `df_sits`, a required input detailed above, must be present within `density_sightings` and those names must match exactly. (That is, `density_sightings` can have many more columns than does `df_sit`, but *all* columns from the latter must be present in the former.) This is needed in order to predict the Effective Strip Width of `density_sightings` based on the detection function fitting procedure.  
...
save(df_sits,
     ...
     Rg0,
     abundance_area,
     file='results_subgroup_eez_objects.rds')
```

```{r eez, echo=TRUE, eval=rerun_eez_from_scratch, collapse=TRUE, message=FALSE, warning=FALSE}
results_eez <- 
...
```{r, echo=FALSE, eval=rerun_eez_from_scratch, collapse=TRUE, message=FALSE, warning=FALSE}
...
save(df_sits,
     ...
     Rg0,
     area_nwhi,
     file='results_subgroup_nwhi_objects.rds')
```

```{r nwhi, echo=TRUE, eval=rerun_nwhi_from_scratch, collapse=TRUE, message=FALSE, warning=FALSE}
results_nwhi <- 
  lta_subgroup(df_sits = df_sits,
               truncation_distance = truncation_distance,
               ss = ss,
...
```{r, echo=FALSE, eval=rerun_nwhi_from_scratch, collapse=TRUE, message=FALSE, warning=FALSE}
...
The analyses above fit detection functions without using any environmental covariates. Here we demonstrate how the results might change if covariates *are* used. Let's re-run the NWHI analysis with two example covariates: `Bft` sea state (numeric) and `EffType` (categorical, or `factor`). For expediency will only use 100 bootstraps.  


```{r nwhi_covar, echo=TRUE, eval=rerun_covar_from_scratch, collapse=TRUE, message=FALSE, warning=FALSE}
results_nwhi_covar <- 
  lta_subgroup(df_sits = df_sits,
               truncation_distance = truncation_distance,
               ss = ss,
               ...
               iterations = 100,
               seed = 123,
               output_dir = 'subgroup_nwhi_covar/',
               toplot = TRUE,
               verbose = TRUE)
```

```{r, echo=FALSE, eval=rerun_covar_from_scratch, collapse=TRUE, message=FALSE, warning=FALSE}
save(results_nwhi_covar, file='results_subgroup_nwhi_covar.RData')
```

```{r, echo=FALSE, eval=TRUE, collapse=TRUE, message=FALSE, warning=FALSE}
load('results_subgroup_nwhi_covar.RData')
```

Now review how these covariate-based results compare to the original run: 

```{r, echo=TRUE, eval=TRUE, collapse=TRUE, message=FALSE, warning=FALSE}
# Point estimates
results_nwhi_covar$estimate %>% filter(population == 'NWHI')

# Ranked detection functions
results_nwhi_covar$df$best_models
```

See [here](#dfsets) for details on how to provide your `df_settings` input to meet your needs, and see [here]({#customcovars) to learn how to develop custom covariates to add to your datasets.  

## Behind the scenes {-}  

This function performs the following operations:

1. Fits a detection function to `df_sits` using the `LTabundR` function `df_fit()`. That model will be used to estimate effective strip width (ESW) for each sighting in `detection_sightings` (and bootstraps thereof).  If specified by the `df_settings` input, the detection function will be modeled with environmental covariates, and the best-fitting function will be determined by stepwise model-fitting and comparison of AICc values. (The same procedure as is used in the [group-based `lta()` function](#lta).)  
...
10. Estimates density for each population using the point estimates of effective strip width, subgroup size, *g(0)*, and the encounter rate.  

11. Estimates abundance for each population by scaling the density estimate by the provided `abundance_area` input.  

12. Creates a bootstrap-resampled distribution of the density estimates by iteratively drawing values (without replacement) from the resampled distributions of the constituent parameters of the density equation.  
