...
The input `sgid` refers to the unique subgroup ID assigned, which you can find when you look at the subgroup data with `subgroup_explorer()` (described above). You can supply a single `sgid` or a vector of `sgid`'s.   
...
To add to this pool of sightings, we also included Phase 1 subgroup detections from 2011 to 2017 based on the PC protocol. Specific criteria: PC Protocol is `Phase 1`, `OnEffort` is `TRUE`,  `EffType` can be `"S"`, `"F"`, or `"N"`; no mixed-species schools; no sightings past beam; detection made by primary observer.   
...
We then use the `LTabundR` function `subgroup_subgroups()` to turn these observer-level detections into a single row for each subgroup, with group sizes averaged using the geometric mean:  
...
library(readxl)
fkw <- read_xlsx("/Users/ekezell/Downloads/FKWSummary_2017analysis.xlsx", sheet=4)
fkw %>% names
names(fkw)[20] <- 'why'
#i <- sapply(fkw$why, function(x){grepl('primary', x)}) %>% c() %>% which
#fkw[i,]
fkw <- fkw %>% select(Date, SightNo, Phase, SubGrp, use=`Use?`, why)
fkw
(dt <- stringr::str_pad(as.character(fkw$Date), width=6, side='left', pad='0'))
fkw$Date <- paste0('20', substr(dt,5,6), '-', substr(dt,1,2), '-', substr(dt,3,4))

fkw$sgid <- 
  paste0(gsub('-','',fkw$Date),'-',
       stringr::str_pad(fkw$SightNo, width=3, side='left',pad='0'),'-',
       fkw$SubGrp)

fkw2join <- fkw %>% select(sgid, use, why)
ss2join <- 
  ss %>% 
  mutate(sgid = substr(sgid, 6, nchar(sgid))) %>% 
  select(Cruise, OnEffort:Bft, sgid, stratum, Phase, GSBest_geom_valid)

df <- full_join(ss2join, fkw2join, by='sgid')
df

df %>% filter(use == 'NO' & !is.na(OnEffort))
df %>% filter(use == 'YES' & is.na(OnEffort))
df %>% filter(is.na(use))
df$use %>% table

events %>% filter(sgid == '1705-20170912-056-C') %>% as.data.frame

alb <- 
  fkw %>% 
  mutate(sgid = paste0(Date,SightNo,Phase,SubGrp, sep=' ')) %>% 
  filter(Phase %in% c(1, 2)) %>% 
  filter(use == 'YES') %>% 
  group_by(Phase, Date) %>% 
  summarize(ALB = length(unique(sgid))) %>% 
...
ltabundr <-
  ss %>% 
  group_by(Phase, Date) %>% 
  summarize(ltabundr = length(unique(sgid))) %>% 
  mutate(Phase = as.character(Phase)) %>% 
  as.data.frame

# DISCREPANCIES
  full_join(alb, ltabundr, by=c('Phase', 'Date')) %>% 
  mutate(ALB = replace_na(ALB, 0)) %>% 
  mutate(ltabundr = replace_na(ltabundr, 0)) %>% 
  filter(ALB != ltabundr) %>% 
  print
  ...
  if(TRUE){
    fkw %>% 
      filter(Date == dti, Phase == 1) %>% 
      mutate(sgid = paste(Date,SightNo,Phase,SubGrp, sep=' - ')) %>% 
      group_by(sgid) %>% summarize(use = use[1]) %>% 
      as.data.frame %>% 
      print
    ss %>% 
      filter(Date == dti) %>% 
      select(Phase, Cruise, Date, OnEffort, EffType, Bft, PerpDist, GSBest_geom_valid, ObsStd, sgid, stratum_OtherCNP) %>% 
      #group_by(sgid) %>% tally() %>% 
      print
  }
```
...
cruz150 <- process_subgroups(cruz150, verbose=FALSE)
```

```{r, echo=FALSE, eval=rerun_from_scratch, collapse=TRUE, message=FALSE, warning=FALSE}
save(cruz150, file='subgroup_cruz.RData')
```

```{r, echo=FALSE, eval=TRUE, collapse=TRUE, message=FALSE, warning=FALSE}
load('subgroup_cruz.RData')
```
...
**Sighting 033** subgroups were automatically placed in the NWHI population, but [Bradford et al. (2020)](https://repository.library.noaa.gov/view/noaa/24756) has them in the Pelagic population.  
...
**Sighting 039** has a subgroup that was in fact seen by the primary observer, according to Bradford et al. (2020):  
...
**Sighting 073** has a subgroup that was in fact seen by the primary observer, according to Bradford et al. (2020):  
...
# SCRATCH
library(readxl)
fkw <- read_xlsx("/Users/ekezell/Downloads/FKWSummary_2017analysis.xlsx", sheet=3)
fkw %>% names
fkw <- fkw %>% select(SightNo = `Sighting no.`, Population, tot=`Phase 1TOT`, est=`Phase 1EST`, n=`n pelagic`)
fkw$n %>% sum
fkw %>% nrow
fkw %>% filter(n>0)

density_sightings %>% 
  select(Cruise, Date, SightNo, population, GSBest_geom) %>% 
  arrange(Date) %>% 
  group_by(Cruise, Date, SightNo, population) %>% 
  summarize(n=n()) #%>% pull(n) %>% sum()

density_sightings$SightNo %>% unique %>% sort
siti <-   cruzi$cohorts$all$subgroups$subgroups %>% 
    filter(SightNo == '073', Phase == 1)
siti
siti$sgid
```
...
sightings_nwhi <- density_sightings %>% filter(stratum_NWHI == TRUE)
```
